...
"""
    ...
    def print_option(self, number: int, text: str, details: str = ""):
        """Print menu option."""
        print(
            f"{Colors.OKCYAN}{Colors.BOLD}[{number}]{Colors.ENDC} {Colors.OKBLUE}{text}{Colors.ENDC}"
        )
        if details:
    ...
    def get_input(
        self, prompt: str, valid_options: Optional[List[str]] = None, allow_navigation: bool = True
    ) -> str:
        ...
        while True:
            ...
            if allow_navigation and user_input in nav_options:
                return user_input.upper()  # Return uppercase for navigation
            print(
                f"{Colors.FAIL}Invalid option. Please choose from: {', '.join(all_valid)}{Colors.ENDC}"
            )
    ...
    def select_datasets(self) -> Tuple[List[Tuple[str, str]], bool]:
        ...
        while True:
            ...
            print(
                f"{Colors.OKCYAN}[S]{Colors.ENDC} {Colors.OKBLUE}Select specific datasets{Colors.ENDC}"
            )
    ...
    def _select_specific_datasets(
        self, dataset_list: List[Tuple[str, Dict]]
    ...
    def configure_test_params(self) -> Dict:
        ...
        print(
            f"{Colors.OKBLUE}K values for Recall@K (comma-separated, default: 3,5,10):{Colors.ENDC}"
        )
    ...
    def prepare_datasets(self, datasets: List[Tuple[str, str]]) -> bool:
        ...
        for dataset_name in missing_datasets:
            ...
            confirm = self.get_input(
                f"Start download for {dataset_name}? (y/n):", valid_options=["y", "n", "yes", "no"]
            )
            ...
            if result.returncode != 0:
                ...
                retry = self.get_input(
                    "\nContinue with other datasets? (y/n):", valid_options=["y", "n", "yes", "no"]
                )
    ...
    def run_benchmark(
        self, datasets: List[Tuple[str, str]], configs: List[str], mode: str, params: Dict
    ) -> bool:
    ...
    def main_menu(self) -> bool:
        ...
        while True:
            ...
            self.print_option(
                1, "Run Performance Tests", "Execute retrieval benchmarks with metrics"
            )
            self.print_option(
                2, "Download & Prepare Datasets", "Download and prepare benchmark datasets"
            )
    ...
    def full_benchmark_menu(self) -> bool:
        ...
        while True:
            ...
            self.print_option(
                2, "Extended Benchmark", "Run extended benchmark with additional metrics"
            )
    ...
    def run_quick_test(self, retrieval_only: bool):
        """Run quick test."""
        print(
            f"{Colors.OKCYAN}Running quick test (retrieval_only={retrieval_only})...{Colors.ENDC}"
        )
...
def main():
    ...
    parser.add_argument(
        "--auto", action="store_true", help="Skip interactive mode and run with provided args"
    )
    parser.add_argument(
        "--dataset",
        "--datasets",
    ...
    parser.add_argument(
        "--mode",
        choices=["retrieval-only", "full", "ingestion-only"],
        default="retrieval-only",
        help="Test mode for auto mode",
    )
    ...
    parser.add_argument(
        "--output", default="results/benchmark_runs", help="Output directory for auto mode"
    )

...
def update_vector_store_metadata(
    collection, doc_id: str, collection_id: str, dry_run: bool = False
) -> int:
    ...
    try:
        ...
        results = collection.get(
            where={"filename": {"$eq": doc_id}}, include=["metadatas", "documents", "ids"]
        )
        ...
        if count > 0 and not dry_run:
            try:
                ...
                logger.info(
                    f"Updated {count} chunks for document {doc_id} with collection_id={collection_id}"
                )
            except Exception as e:
...
def backfill_from_metadata_manager(
    ...
    try:
        ...
        with metadata_manager._lock:
            ...
            if collection_id:
                cur.execute(
                    "SELECT run_id FROM ingestion_runs WHERE id = ? LIMIT 1", (collection_id,)
                )
            ...
            for run_id in run_ids:
                ...
                for file_path in file_paths:
                    ...
                    updated = update_vector_store_metadata(
                        collection, doc_id=filename, collection_id=run_id, dry_run=dry_run
                    )
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force update even if metadata already exists"
    )

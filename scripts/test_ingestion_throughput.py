...
class IngestionTester:
    ...
    def measure_ingestion(self, fast_pass: bool = True) -> Dict[str, Any]:
        ...
        logger.info(f"Data to ingest: {data_size['total_gb']:.2f} GB ({data_size['file_count']} files)")
        ...
        try:
            ...
            seconds_per_gb = ingestion_time / data_size['total_gb'] if data_size['total_gb'] > 0 else 0
            ...
            compression_ratio = data_size['total_bytes'] / storage_estimate['total_bytes'] if storage_estimate['total_bytes'] > 0 else 0
            ...
            logger.info(f"Ingestion completed in {ingestion_time:.2f}s ({minutes_per_gb:.2f} min/GB)")
...
def main():
    ...
    parser.add_argument("--fast-pass", action="store_true", default=True,
                       help="Use fast-pass ingestion (default: True)")
    parser.add_argument("--deep-ingest", action="store_true",
                       help="Use deep ingestion instead of fast-pass")

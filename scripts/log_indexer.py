...
def ensure_db(db_path: Path):
    ...
    cur = conn.cursor()
    # Create FTS table
    cur.execute('''CREATE TABLE IF NOT EXISTS log_meta (id INTEGER PRIMARY KEY, key TEXT UNIQUE, value TEXT)''')
    cur.execute('''CREATE VIRTUAL TABLE IF NOT EXISTS logs_fts USING fts5(message, level, component, timestamp, trace_id, context, logfile);''')
    conn.commit()
    return conn
...
def set_offset(conn, logfile: str, value: int):
    cur = conn.cursor()
    cur.execute('INSERT OR REPLACE INTO log_meta (key, value) VALUES (?, ?)', (f'offset:{logfile}', str(value)))
    conn.commit()
    ...
    with open(logfile, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            try:
                ...
                rec = {'message': line, 'timestamp': datetime.utcnow().isoformat(), 'level': 'INFO', 'component': 'log'}
            ...
            cur.execute('INSERT INTO logs_fts (message, level, component, timestamp, trace_id, context, logfile) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
def main():
    ...
    parser.add_argument('--db', required=False, default='./logs/index/logs.db', help='SQLite DB path for index')

...
Usage: python scripts/run_reranker_and_system_metrics_all.py
"""
...
def run_reranker(ds, index_dir, queries, top_k=10):
    ...
    cmd = ['python','scripts/run_reranker_eval.py','--index-dir', index_dir, '--queries', queries, '--top-k', str(top_k)]
...
def run_system_metrics(ds, corpus, index_dir, queries, top_k=50, limit=0):
    ...
    cmd = ['python','scripts/system_metrics.py','--corpus', corpus, '--index-dir', index_dir, '--queries', queries, '--top-k', str(top_k)]
    if limit:
        cmd += ['--limit', str(limit)]
    subprocess.run(cmd, check=True)
    # find latest file

...
def run_worker(index_dir, queries, output, top_k, mode):
    cmd = [
        'python', 'scripts/worker_retrieve.py',
        '--index-dir', index_dir,
        '--queries', queries,
        '--output', output,
        '--top-k', str(top_k),
...
def run_eval(index_dir, queries, top_k):
    ...
    # Compute metrics
    # Use scripts/calc_metrics_from_run.py to compute Recall@10
    res_no = subprocess.run(['python', 'scripts/calc_metrics_from_run.py', '--run', out_no, '--qrels', queries.replace('queries.jsonl','qrels/test.tsv'), '--k', '10'], capture_output=True, text=True)
    res_with = subprocess.run(['python', 'scripts/calc_metrics_from_run.py', '--run', out_with, '--qrels', queries.replace('queries.jsonl','qrels/test.tsv'), '--k', '10'], capture_output=True, text=True)

...
class BenchmarkRunner:
    ...
    def _run_with_retries(self, cmd: List[str], cwd: str = None, max_retries: int = 3, backoff: float = 2.0):
    ...
    def run(self, run_ingest_first: bool = True, timeout: int = None):
        ...
        for ds, rc, ab in combos:
            ...
            if run_ingest_first:
                logger.info(f"Running ingestion for dataset {ds_name} (path={ds.get('path')}) as subprocess")
                ingest_cmd = [
                    sys.executable,
                    'scripts/test_ingestion_throughput.py',
                    '--data-folder', ds.get('path'),
                    '--output', str(run_dir / 'ingestion_result.json'),
                    '--fast-pass'
                ]
                succeeded, attempts, err = self._run_with_retries(ingest_cmd, cwd=None, max_retries=self.max_retries, backoff=self.retry_backoff)
                if succeeded:
                    try:
                        with open(run_dir / 'ingestion_result.json', 'r', encoding='utf-8') as f:
                            ingestion_results = json.load(f)
                    except Exception as ex:
                        logger.error(f"Failed to load ingestion_result.json: {ex}")
                        ingestion_results = {'success': False, 'error': 'result_file_missing', 'attempts': attempts}
                else:
            ...
            run_results = None
            logger.info(f"Running {self.mode} tests for dataset {ds_name} with config {rc_name} and ablation {ab_name} (subprocess)")
            questions_path = ds.get('questions', 'test_questions.json')
            ground_truth = ds.get('ground_truth', None)
            test_cmd = [
                sys.executable, 'scripts/run_rag_tests.py',
                '--questions', questions_path,
                '--data-folder', ds.get('path'),
                '--mode', self.mode,
                '--k-values', ','.join(str(k) for k in self.k_values),
                '--output', str(run_dir / 'test_results.json')
            ]
            if ground_truth:
            ...
            if ds.get('hard_limit'):
                ...
                succeeded, attempts, err = self._run_with_retries(test_cmd, cwd=None, max_retries=self.max_retries, backoff=self.retry_backoff)
                if succeeded:
                    try:
                        with open(run_dir / 'test_results.json', 'r', encoding='utf-8') as f:
                            run_results = json.load(f)
                    except Exception as ex:
                        logger.error(f"Failed to load test_results.json: {ex}")
                        run_results = {'success': False, 'error': 'result_file_missing', 'attempts': attempts}
                else:
            ...
            combined_results = {
                'metadata': {
                    ...
                    'attempts': {
                        'ingestion_attempts': ingestion_results.get('attempts') if isinstance(ingestion_results, dict) else None,
                        'test_attempts': run_results.get('attempts') if isinstance(run_results, dict) else None
                    }
                },
                'results': run_results
            }
            ...
            try:
                ingestion_minutes_per_gb = combined_results['metadata'].get('ingestion_results', {}).get('ingestion', {}).get('minutes_per_gb')
            except Exception:
                ingestion_minutes_per_gb = None
            ...
            row = {
                ...
                'avg_recall_at_k_10': combined_results['results'].get('metadata', {}).get('avg_recall_at_k_10', 0),
                'avg_ndcg_at_k_10': combined_results['results'].get('metadata', {}).get('avg_ndcg_at_k_10', 0),
                'avg_retrieval_latency_p50_ms': combined_results['results'].get('metadata', {}).get('avg_retrieval_latency_p50_ms', 0),
                'avg_answer_relevance': combined_results['results'].get('metadata', {}).get('avg_answer_relevance', 0),
                'ingestion_minutes_per_gb': ingestion_minutes_per_gb
            }
            # Append attempts and error message if present
            row['ingestion_attempts'] = combined_results['metadata'].get('attempts', {}).get('ingestion_attempts')
            row['test_attempts'] = combined_results['metadata'].get('attempts', {}).get('test_attempts')
...
def main():
    ...
    parser.add_argument('--configs', required=True,
                        help='Path to JSON config file with retrieval configs')
    parser.add_argument('--ablations', required=False, default=None,
                        help='Path to JSON file with ablation configs')
    parser.add_argument('--k-values', default='5,10,20', help='Comma separated K values')
    parser.add_argument('--mode', default='retrieval-only', choices=['retrieval-only', 'full', 'ingestion-only'])

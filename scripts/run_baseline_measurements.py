...
def run_all_baselines(dataset: str, num_queries: int = 100, warmup: int = 10) -> Dict:
    ...
    print(
        f"  Hardware: {results['hardware']['processor']}, {results['hardware']['cpu_count']} cores"
    )
...
def print_comparison_table(results: Dict):
    ...
    print(
        f"Queries: {results['experiment_info']['num_queries']} (after {results['experiment_info']['warmup_queries']} warmup)"
    )
...
def main():
    ...
    parser.add_argument(
        "--output", default="results/baseline_comparison.json", help="Output JSON file"
    )

...
class ScaffoldGenerator:
    ...
    def __init__(
        ...
        if self.use_clustering:
            ...
            self.clusterer = SemanticClusterer(
                method=clustering_method, min_cluster_size=scaffold_size
            )
        else:
            self.clusterer = None
    ...
    def generate_scaffolds(
        ...
        logger.info(
            f"Generating scaffolds from {len(chunks_df)} chunks (clustering={self.use_clustering})"
        )
        ...
        scaffold_groups = self._group_chunks_into_scaffolds(
            enriched_chunks, chunk_ids, chunk_embeddings
        )
        scaffolds_data = self._create_scaffold_data(
            scaffold_groups, enriched_chunks, chunks_df, text_column
        )
    ...
    def _enrich_chunks_if_needed(
        self, chunks_df: pd.DataFrame, text_column: str
    ) -> List[Dict[str, Any]]:
    ...
    def _group_chunks_into_scaffolds(
        ...
        if self.use_clustering and self.clusterer and chunk_embeddings is not None:
            try:
                ...
                logger.info(
                    f"Semantic clustering grouped {len(enriched_chunks)} chunks into {len(groups)} scaffolds"
                )
                return groups
        ...
        if current_group:
            groups.append(current_group)
        logger.info(
            f"Sequential grouping created {len(groups)} scaffolds from {len(enriched_chunks)} chunks"
        )
        return groups
    ...
    def _create_scaffold_data(
        ...
        for group_idx, group in enumerate(scaffold_groups):
            ...
            scaffold_data = {
                ...
                "original_size": sum(
                    len(str(original_chunks_df.iloc[i].get(text_column, ""))) for i in group
                ),
                "compressed_size": len(merged_summary),
                "original_token_count": sum(
                    int(
                        original_chunks_df.iloc[i].get("token_count", 0)
                        or len(str(original_chunks_df.iloc[i].get(text_column, "")).split())
                    )
                    for i in group
                ),
                "compressed_token_count": len(str(merged_summary).split()),
                "compression_ratio": (
                    sum(len(str(original_chunks_df.iloc[i].get(text_column, ""))) for i in group)
                    / len(merged_summary)
                    if len(merged_summary) > 0
                    else 0
                ),
            }
    ...
    def _generate_scaffold_embeddings(
        self, scaffolds_data: List[Dict[str, Any]]
    ) -> List[List[float]]:
...
def _build_chunks_summary_from_df(
        ...
        for _, row in scaffolds_df.iterrows():
            for cid in row["chunk_ids"]:
                entry = {
                    ...
                    "chunk_id": cid,
                    "scaffold_group": row["group_index"],
                }
                if input_chunks_df is not None and id_column in input_chunks_df.columns:
                    ...
                    if not matched.empty:
                        ...
                        entry["token_count"] = int(
                            m.get("token_count", 0) or len(str(m.get("text", "")).split())
                        )
...
def save_scaffold_run(
    ...
    try:
        ...
        manager.record_scaffold_run(
            run_id,
            str(run_dir),
            model_version or "",
            int(manifest["scaffold_count"]),
            str(manifest_path),
        )
        for s in chunks_summary:
            manager.add_scaffold_mapping(
                run_id, s["scaffold_id"], s["chunk_id"], {"group_index": s.get("scaffold_group")}
            )
    except Exception:
...
def create_scaffolds_from_parquet(
    ...
    generator = ScaffoldGenerator(
        enricher=enricher, embedding_generator=embedding_generator, scaffold_size=scaffold_size
    )
    scaffolds_result = generator.generate_scaffolds(
        chunks_df, text_column=text_column, id_column=id_column
    )

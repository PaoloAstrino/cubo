...
class Paths(BaseSettings):
    ...
    deep_output_dir: Path = Field(default=Path("./data/deep"), description="Output for deep ingestion")
    fast_pass_output_dir: Path = Field(default=Path("./data/fastpass"), description="Output for fast pass")
...
class ChunkingSettings(BaseSettings):
    ...
    chunk_overlap_sentences: int = Field(default=1, ge=0, description="Number of sentences to overlap")
    
    # Legacy / Benchmarking
    benchmark_chunk_size: int = Field(default=1200, description="Token size for naive benchmarking if needed")
...
class RetrievalSettings(BaseSettings):
    ...
    bm25_normalization_factor: float = Field(default=15.0, description="Empirical max score for normalization")
    ...
    initial_retrieval_multiplier: int = Field(default=5, description="Multiplier for initial candidate retrieval")
    complexity_length_threshold: int = Field(default=12, description="Query length threshold for complexity")
    ...
    semantic_weight_detailed: float = Field(default=0.1, description="Semantic weight for detailed queries")
    bm25_weight_detailed: float = Field(default=0.9, description="BM25 weight for detailed queries")
    
    # Scoring Thresholds
    min_bm25_threshold: float = Field(default=0.05, description="Min normalized BM25 score for boosting")

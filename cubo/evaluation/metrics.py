...
class IRMetricsEvaluator:
    ...
    def compute_ndcg_at_k(
        ...
        if relevance_scores:
            ...
            sorted_rels = sorted(
                [relevance_scores.get(doc_id, 1.0) for doc_id in relevant_ids], reverse=True
            )[:k]
        else:
    ...
    def evaluate_retrieval(
        ...
        for k in k_values:
            recall_at_k[k] = IRMetricsEvaluator.compute_recall_at_k(relevant_ids, retrieved_ids, k)
            precision_at_k[k] = IRMetricsEvaluator.compute_precision_at_k(
                relevant_ids, retrieved_ids, k
            )
            ndcg_at_k[k] = IRMetricsEvaluator.compute_ndcg_at_k(relevant_ids, retrieved_ids, k)

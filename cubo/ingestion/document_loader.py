...
class DocumentLoader:
    ...
    def _load_and_process_standard(
        self, file_path: str, chunking_config: dict = None
    ) -> List[dict]:
    ...
    def _embed_file_metadata(self, chunks: List[dict], file_path: str) -> None:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            chunk["token_count"] = chunk.get("sentence_token_count") or len(
    ...
    def _log_chunking_results(self, file_path: str, chunks: List[dict], cfg: dict):
        ...
        logger.info(
            f"Loaded and chunked {os.path.basename(file_path)} into "
    ...
    def load_documents_from_folder(self, folder_path: str) -> List[dict]:
        ...
        if not supported_files:
            logger.warning(
                f"No supported files {self.supported_extensions} found in the specified folder or its subfolders."
            )
            return []

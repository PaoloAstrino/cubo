        ...
        try:
            if change["embedding"] is not None:
                coll.add(
                    ...
                    embeddings=[change["embedding"]],
                    metadatas=[change["meta"]],
                )
            else:
                coll.add(
                    ids=[change["new_id"]], documents=[change["doc"]], metadatas=[change["meta"]]
                )
            ...
            logger.error(
                f"Failed to apply change for {change['old_id']} -> {change['new_id']}: {e}"
            )
            raise
...
def main():
    ...
    for idx, old_id in enumerate(ids):
        ...
        logger.debug(
            f"Index {idx}: old_id={old_id}, file_hash={meta.get('file_hash')}, filename={filename}"
        )
        if meta.get("sentence_index") is not None:
            new_id = f"{base}_s{meta.get('sentence_index')}"
        elif meta.get("page") is not None and meta.get("table_index") is not None:

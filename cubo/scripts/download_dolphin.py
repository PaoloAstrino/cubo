"""Lightweight stub for Dolphin downloader used by unit tests.

This module intentionally avoids heavy imports and network calls.
Only helper functions required by tests are implemented.
"""
...
def _resolve_hf_revision() -> Optional[str]:
...
def download_dolphin_model() -> bool:
    """Stubbed downloader: not available in CI/test environment.

    Tests should use `_resolve_hf_revision` only; this function exists to
    preserve the public API but does not perform network operations.
    """
    raise RuntimeError("Dolphin downloader is not available in this environment")


def test_dolphin_model() -> bool:
    """Stub test that always returns False (no local model present)."""
    return False
#!/usr/bin/env python3
"""

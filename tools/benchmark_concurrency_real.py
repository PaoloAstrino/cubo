...
async def run_concurrent_benchmark_real(
    ...
    logger.info(f"Starting REAL concurrency benchmark: {num_workers} workers, "
    ...
    candidates = [Path(index_dir), Path('data/beir_index_scifact'), Path('data/faiss_test'), Path('beir_index_scifact')]
    resolved = None
    for c in candidates:
        if c and c.exists():
            resolved = str(c)
            break
    if resolved is None:
        raise FileNotFoundError(f"No FAISS index found (checked {candidates}); set --index-dir to a valid index")
    ...
    tasks = [
        query_workload_real(
            i, queries_per_worker, retriever, test_queries,
            results_queue, lock_timings
        )
    ...
    if concurrent_ingestion and ingestion_docs:
        ...
        if IngestionPipeline is not None:
            ...
            tasks.append(
                ingestion_workload_real(ingestion_docs, ingestion_pipeline, results_queue)
            )
        else:
            logger.info("Concurrent ingestion requested but unavailable; continuing without ingestion")
...
def main():
    ...
    parser.add_argument(
        "--ingestion-docs",
        type=Path,
        help="Directory with documents to ingest during benchmark"
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    # Run benchmark
    results = asyncio.run(run_concurrent_benchmark_real(
        args.index_dir,
        args.num_workers,

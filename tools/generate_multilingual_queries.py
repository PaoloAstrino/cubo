...
def generate_language_data(lang_code, lang_name, data_map):
    ...
    for i in range(55):
        all_queries.append({
            "query_id": f"q_hard_{i}",
            "text": data_map['hard_query'],
            "relevant_docs": ["none"]
        })
    ...
    print(f"Generated {len(documents)} docs and {len(all_queries)} queries for {lang_name} ({lang_code}).")

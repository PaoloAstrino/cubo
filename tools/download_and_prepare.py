...
Usage:
    python tools/download_and_prepare.py --dataset ultradomain --skip-existing
    python tools/download_and_prepare.py --dataset ultradomain --sample-percent 2
    python tools/download_and_prepare.py --models bge-base-en-v1.5 --verify
"""
...
class DatasetDownloader:
    ...
    def __init__(
        self,
        manifest_path: str = "configs/datasets_manifest.json",
        output_dir: str = "data",
    ...
    def _extract_archive(self, archive_path: Path, extract_dir: Path) -> bool:
        ...
        try:
            if archive_path.suffix == ".gz" and archive_path.stem.endswith(".tar"):
                ...
                self._print_status(
                    f"Unsupported archive format: {archive_path.suffix}", Colors.FAIL
                )
                return False
    ...
    def download_dataset(self, dataset_name: str, url: Optional[str] = None) -> bool:
        ...
        if not dataset_info and not url:
            self._print_status(
                f"Dataset '{dataset_name}' not found in manifest and no URL provided", Colors.FAIL
            )
            return False
        ...
        if download_url == "local":
            self._print_status(
                f"Dataset '{dataset_name}' is local - skipping download", Colors.OKGREEN
            )
            return True
        ...
        if exists and self.skip_existing:
            self._print_status("Dataset exists and skip_existing=True - skipping", Colors.WARNING)
            return True

        if exists and not self.force:
            self._print_status("Dataset exists. Use --force to re-download", Colors.WARNING)
            return True

        # Force re-download
        if self.force and dataset_dir.exists():
            self._print_status("Force re-download - removing existing directory", Colors.WARNING)
        ...
        if self.verify and dataset_info:
            ...
            if missing_files:
                self._print_status(
                    f"Warning: Missing expected files: {missing_files}", Colors.WARNING
                )
            else:
                self._print_status("All expected files verified", Colors.OKGREEN)
            ...
            if dataset_name == "beir":
                ...
                if subdirs:
                    self._print_status(
                        f"Reorganizing BeIR files from subdirectory: {subdirs[0].name}",
                        Colors.WARNING,
                    )
                    ...
                    if len(subdirs) == 1:
                        ...
                        self._print_status("BeIR files moved to root directory", Colors.OKGREEN)
    ...
    def _handle_manual_download(self, dataset_name: str, dataset_info: Dict) -> bool:
        ...
        self._print_status(
            f"Manual download required for {dataset_name}", Colors.WARNING, bold=True
        )
        ...
        if dataset_dir.exists():
            ...
            if found_files:
                self._print_status(
                    f"Found {len(found_files)} expected files: {found_files}", Colors.OKGREEN
                )
                if len(found_files) == len(expected_files):
                    ...
                    self._write_marker(dataset_dir, marker_metadata)
                    self._print_status(
                        f"Dataset '{dataset_name}' ready!", Colors.OKGREEN, bold=True
                    )
                    return True
                else:
                    self._print_status(
                        f"Missing files: {[f for f in expected_files if f not in found_files]}",
                        Colors.WARNING,
                    )
            else:
    ...
    def _download_huggingface_dataset(
        self, dataset_name: str, repo_id: str, dataset_info: Dict
    ) -> bool:
    ...
    def _download_huggingface_hub_snapshot(
        self, dataset_name: str, repo_id: str, dataset_info: Dict
    ) -> bool:
    ...
    def _download_large_huggingface_dataset(
        self, dataset_name: str, repo_id: str, dataset_info: Dict
    ) -> bool:
        """Download large HuggingFace datasets with batch processing to avoid timeouts."""
        self._print_status(
            f"Downloading large dataset {repo_id} in batches to avoid timeouts", Colors.WARNING
        )
        ...
        try:
            ...
            if expected_files:
                filtered_files = [
                    f for f in all_files if any(f.endswith(exp) for exp in expected_files)
                ]
                if filtered_files:
                    all_files = filtered_files
                    self._print_status(
                        f"Filtered to {len(all_files)} expected files", Colors.OKBLUE
                    )
            ...
            for i in range(0, len(all_files), batch_size):
                ...
                self._print_status(
                    f"Downloading batch {i//batch_size + 1}/{(len(all_files) + batch_size - 1)//batch_size}: {batch}",
                    Colors.OKCYAN,
                )
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if attempt < max_retries - 1:
                                ...
                                self._print_status(
                                    f"Failed to download {file_path} (attempt {attempt + 1}/{max_retries}): {e}. Retrying in {wait_time}s...",
                                    Colors.WARNING,
                                )
            ...
            self._print_status(
                f"Successfully downloaded {successful_downloads}/{len(all_files)} files",
                Colors.OKGREEN,
            )
    ...
    def _download_huggingface_datasets_library(
        self, dataset_name: str, repo_id: str, dataset_info: Dict
    ) -> bool:
        ...
        try:
            ...
            self._print_status(
                "datasets library not installed. Install with: pip install datasets", Colors.FAIL
            )
            return False
        ...
        self._print_status(
            f"Downloading {len(subsets)} subsets in batches: {subsets[:3]}...", Colors.OKBLUE
        )
        ...
        try:
            ...
            for i in range(0, len(subsets), batch_size):
                ...
                self._print_status(
                    f"Processing batch {i//batch_size + 1}/{(len(subsets) + batch_size - 1)//batch_size}: {batch}",
                    Colors.OKCYAN,
                )
                ...
                for subset in batch:
                    ...
                    try:
                        ...
                        dataset = load_dataset(
                            repo_id, subset, download_mode=DownloadMode.FORCE_REDOWNLOAD
                        )
                        ...
                        for split_name, split_data in dataset.items():
                            if hasattr(split_data, "to_pandas"):
                                ...
                                self._print_status(
                                    f"Saved {subset}/{split_name} ({len(df)} rows)", Colors.OKGREEN
                                )
                            else:
            ...
            self._print_status(
                f"Dataset '{dataset_name}' with {len(all_data)} subsets ready!",
                Colors.OKGREEN,
                bold=True,
            )
            return True
    ...
    def create_sample(
        self, dataset_name: str, sample_percent: int = 2, include_ground_truth: bool = True
    ) -> bool:
        """Create a percentage sample of a dataset."""
        self._print_status(
            f"Creating {sample_percent}% sample of '{dataset_name}'", Colors.HEADER, bold=True
        )
        ...
        self._print_status(
            f"Total files: {total_files}, Sample size: {sample_count}", Colors.OKBLUE
        )
    ...
    def verify_model(self, model_name: str) -> bool:
        ...
        if model_info.get("source") == "local":
            if model_dir.exists():
                ...
                try:
                    ...
                    if expected_dim and actual_dim != expected_dim:
                        ...
                        self._print_status(
                            f"Model verified: {actual_dim}-dimensional embeddings", Colors.OKGREEN
                        )
            ...
            try:
                ...
                actual_dim = model.get_sentence_embedding_dimension()
                self._print_status(
                    f"Model downloaded and verified: {actual_dim}-dimensional", Colors.OKGREEN
                )
                return True
            except Exception as e:
                self._print_status(f"Download failed: {e}", Colors.FAIL)
...
Examples:
  ...
  python tools/download_and_prepare.py --models embeddinggemma-300m --verify

  # Force re-download
  python tools/download_and_prepare.py --dataset ultradomain --force
        """,
    )
    ...
    parser.add_argument(
        "--output", default="data", help="Output directory for datasets (default: data)"
    )
    parser.add_argument(
        "--manifest",
        default="configs/datasets_manifest.json",
        help="Path to datasets manifest JSON",
    )
    parser.add_argument(
        "--skip-existing", action="store_true", help="Skip download if dataset already exists"
    )
    ...
    parser.add_argument(
        "--sample-percent", type=float, help="Create a sample of this percentage (e.g., 2 for 2%%)"
    )
    parser.add_argument(
        ...
        for dataset_spec in args.datasets:
            ...
            if args.sample_percent:
                if not downloader.create_sample(
                    dataset_name, args.sample_percent, args.include_ground_truth
                ):
                    success = False

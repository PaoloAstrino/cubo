...
analysis = {
    ...
    'latency_range': {'min': min(r['latency_mean_ms'] for r in results), 'max': max(r['latency_mean_ms'] for r in results)},
    'latency_increase_factor': max(r['latency_mean_ms'] for r in results) / min(r['latency_mean_ms'] for r in results),
    'monotonic_increase': all(results[i]['latency_mean_ms'] <= results[i+1]['latency_mean_ms'] for i in range(len(results) - 1)),

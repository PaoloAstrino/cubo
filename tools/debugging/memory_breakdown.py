...
def main():
    ...
    try:
        ...
        t1 = time.time()
        gc.collect()
        m['checkpoints'].append({'name': 'embedding_model_loaded', 'rss_mb': rss_mb(), 'time_s': t1 - t0})
    ...
    if eg and texts:
        try:
            ...
            t1 = time.time()
            gc.collect()
            m['checkpoints'].append({'name': 'embeddings_computed', 'rss_mb': rss_mb(), 'time_s': t1 - t0})
    ...
    try:
        ...
        t0 = time.time()
        manager = TieredIndexManager(dimension=len(embeddings[0]) if embeddings else 768, index_dir=Path('results/tmp_index_with_bm25'))
    ...
    try:
        ...
        for nm, dv in zip(names, deltas):
            ax.text(0, ypos + dv/2, f"{nm}: {dv:.1f}MB", va='center', ha='center', color='white', fontsize=8)
            ypos += dv

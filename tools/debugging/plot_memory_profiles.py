...
def plot_single_run(profile: dict, corpus_label: str, output_path: Path):
    ...
    # Plot RSS over time
    ax.plot(profile["elapsed"], profile["rss_mb"], linewidth=1.5, color="#2E86AB", alpha=0.8)
...
def plot_multi_corpus(profiles: dict, output_path: Path):
    ...
    ax.set_ylabel("RSS Memory (MB)", fontsize=12)
    ax.set_title(
        "O(1) Memory Scaling: Constant RSS Across Corpus Sizes", fontsize=14, fontweight="bold"
    )
    ax.grid(True, alpha=0.3)

...
try {
    ...
    $requiredScripts = @("tools/run_beir_adapter.py", "tools/run_reranker_eval.py", "tools/system_metrics.py", "tools/run_ablation.py")
    ...
    $step2Start = Get-Date
    python tools/run_reranker_eval.py `
        --index-dir "results/${Dataset}_bench_index" `
        --queries $queryFile `
        --top-k 10
    $step2End = Get-Date
    ...
    $step3Start = Get-Date
    python tools/system_metrics.py `
        --corpus $corpusFile `
        --queries $queryFile `

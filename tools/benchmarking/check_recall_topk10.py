...
if __name__ == "__main__":
    ...
    t0 = time.time()
    while not (
        getattr(cubo, "retriever", None) and hasattr(cubo.retriever, "retrieve_top_documents")
    ):
        if time.time() - t0 > 60:
    ...
    print("GT doc in top-k:", doc_hit, f"({doc_hit/total:.2f})")
    print("Supporting sentence in contexts:", sent_hit, f"({sent_hit/total:.2f})")

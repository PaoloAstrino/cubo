...
def _build_base_command(dataset, queries, index_dir, top_k, output_file):
    ...
    return [
        ...
        "--corpus",
        f"data/beir/{dataset}/corpus.jsonl",
        "--queries",
        queries,
        "--output",
        output_file,
...
def _print_results(
    dataset, metrics_base, elapsed_base, metrics_rerank, elapsed_rerank, delta_recall
):
...
def measure_reranker_effect(dataset, index_dir, queries, qrels, top_k=50):
    ...
    result = _create_full_result(
        dataset, metrics_base, elapsed_base, metrics_rerank, elapsed_rerank
    )
    _print_results(
        dataset, metrics_base, elapsed_base, metrics_rerank, elapsed_rerank, result["delta_recall"]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--index-dir", help="Index directory (default: results/beir_index_{dataset}_shared)"
    )
    parser.add_argument(
        "--queries", help="Queries file (default: data/beir/{dataset}/queries.jsonl)"
    )

...
datasets = [
    ...
    "ultradomain_medium",
    "ultradomain_legal",
    "ultradomain_politics",
    "ultradomain_agriculture",
    "scidocs",
    "ragbench_merged",
]
...
def find_index_dir(ds):
    # common patterns
    candidates = [
        f"results/beir_index_{ds}",
        f"results/beir_index_{ds}_shared",
        f"results/beir_index_{ds}_topk50",
        f"results/beir_index_{ds}_shared",
    ]
    for c in candidates:
        if os.path.exists(c):
...
def find_queries(ds):
    candidates = [
        f"data/beir/{ds}/queries.jsonl",
        f"data/beir/{ds}/queries_test100.jsonl",
        f"data/beir/{ds}/queries_quick50.jsonl",
    ]
    for c in candidates:
        if os.path.exists(c):
            return c
...
def run_reranker(ds, index_dir, queries, top_k=10):
    ...
    for prefix in [
        f"results/run_no_rerank_topk{top_k}.json",
        f"results/run_with_rerank_topk{top_k}.json",
        f"results/run_no_rerank_topk{top_k}_latencies.json",
        f"results/run_with_rerank_topk{top_k}_latencies.json",
    ]:
    ...
    for ds in datasets:
        ...
        if not index_dir or not queries:
            print(
                "Skipping",
                ds,
                "missing index_dir or queries (index_dir:",
                index_dir,
                "queries:",
                queries,
                ")",

...
def run_e5_queries(
    index_dir: Path, queries: dict, top_k: int = 100
    ...
    for i, (qid, query_text) in enumerate(queries.items()):
        ...
        doc_results = [
            (doc_ids[int(idx)], float(1 - dist)) for idx, dist in zip(indices[0], distances[0])
        ]
...
def main():
    ...
    # Run queries
    results_list, latencies, initial_memory, peak_memory = run_e5_queries(

...
def parse_args():
    ...
    p.add_argument(
        "--limit", type=int, default=None, help="Limit number of docs to index (test mode)"
    )
    ...
    p.add_argument(
        ...
        help="Python binary to use for subprocess",
    )
    return p.parse_args()
...
def main():
    ...
    with open(meta_readme, "w", encoding="utf-8") as rm:
        ...
        rm.write("\n")
        rm.write(
            "To reproduce: activate venv and run the command above (captured in `cmd` field of the JSON)."
        )

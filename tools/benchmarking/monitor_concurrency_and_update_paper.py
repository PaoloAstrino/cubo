...
def wait_for_test_completion(test_file: Path, timeout_minutes: int = 10) -> bool:
    ...
    while time.time() - start < timeout_sec:
        ...
        print(
            f"\r⏳ Waiting for test completion... ({elapsed}s / {timeout_sec}s)", end="", flush=True
        )
        time.sleep(30)
...
def extract_and_update_table():
    ...
    result = subprocess.run(
        [
            ...
            "--smoke-json",
            "results/concurrency/scifact_smoke.json",
            "--full-json",
            "results/concurrency/scifact_full.json",
            "--output-csv",
            "results/concurrency_metrics_table.csv",
            "--output-latex",
            "results/concurrency_metrics_table.tex",
        ],

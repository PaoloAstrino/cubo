...
def measure_faiss_sensitivity_real(
    retriever, queries: List[Dict], nprobe: int, top_k: int = 10
) -> Dict:
...
def run_sensitivity_grid_real(
    retriever, queries: List[Dict], nprobe_values: List[int], top_k: int = 10
) -> List[Dict]:
...
def main():
    ...
    parser.add_argument(
        "--num-samples", type=int, default=30, help="Number of queries to sample for each config"
    )
    ...
    sensitivity_results = run_sensitivity_grid_real(retriever, queries, nprobe_values, args.top_k)

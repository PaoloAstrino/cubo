...
def measure_faiss_search_latency(
    nprobe: int,
    nlist: int,
    disk_type: str = "SATA"
) -> Dict:
...
def analyze_sensitivity(results: List[Dict]) -> Dict:
    ...
    return {
        ...
        "disk_type_sensitivity": {
            "values": disk_effect,
            "relative_impact": disk_effect.get("SATA", 0) / disk_effect.get("NVMe", 1) if "NVMe" in disk_effect else 1.0
        }
    }
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="SATA,NVMe",
        help="Comma-separated disk types"
    )
    parser.add_argument(
        "--output",
        type=Path,

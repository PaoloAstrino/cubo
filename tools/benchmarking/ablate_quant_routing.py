...
def run_retrieval_ablation(dataset: str, run_config: str, index_dir: Path) -> Dict[str, float]:
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, default="scifact", help="BEIR dataset (scifact, fiqa, etc.)"
    )
    parser.add_argument(
        "--runs",
        type=str,
    ...
    parser.add_argument(
        "--index-dir", type=Path, default=Path("faiss_store"), help="FAISS index directory"
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    for config in run_configs:
        ...
        logger.info(
            f"  {config}: Recall@10={metrics['recall@10']:.4f}, nDCG@10={metrics['ndcg@10']:.4f}"
        )

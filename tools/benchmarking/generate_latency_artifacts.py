...
def create_latex_table(results, output_path):
    ...
    for component in ['embedding', 'faiss_search', 'bm25_search', 'fusion', 'rerank']:
        if component in stats:
            ...
            rows.append(f"{comp_name:30s} & {p50:7.1f} & {p95:7.1f} & {p99:7.1f} & {percentage:5.1f} \\\\")
    
    # Add total row
    total = stats['total']
    total_name = r"\textbf{Total}"
    rows.append(f"{total_name:30s} & {total['p50']:7.1f} & {total['p95']:7.1f} & {total['p99']:7.1f} & {100.0:5.1f} \\\\")
...
def main():
    ...
    parser.add_argument('json_file', nargs='?', default='results/latency_breakdown_full_100.json',

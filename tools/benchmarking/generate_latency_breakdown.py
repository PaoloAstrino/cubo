...
def create_breakdown_chart(stats: dict, output_path: Path):
    ...
    # Plot p50
    bars1 = ax1.bar(
        range(len(components)), p50_values, color=colors, edgecolor="black", linewidth=1.5
    )
    ...
    # Plot p95
    bars2 = ax2.bar(
        range(len(components)), p95_values, color=colors, edgecolor="black", linewidth=1.5
    )
...
def create_latex_table(stats: dict) -> str:
    ...
    for component in components:
        ...
        latex_lines.append(f"{comp_name} & {p50:.1f} & {p95:.1f} & {p99:.1f} & {pct:.1f}\\% \\\\")

...
def run_splade_queries(index_dir: Path, queries: dict, top_k: int = 100) -> tuple:
    ...
    for i, (qid, query_text) in enumerate(queries.items()):
        ...
        # Encode query
        inputs = tokenizer(
            [query_text], padding=True, truncation=True, return_tensors="pt", max_length=512
        )
        ...
        if (i + 1) % max(10, len(queries) // 10) == 0:
            ...
            print(
                f"  [{i+1}/{len(queries)}] Avg latency: {avg_lat:.0f}ms, Peak memory: {peak_memory:.1f}GB"
            )
...
def main():
    ...
    # Run queries
    results_list, latencies, initial_memory, peak_memory = run_splade_queries(
        args.index_dir, queries, args.top_k
    )
    ...
    results = {
        ...
        "evaluation": {"recall@10": 0.0, "ndcg@10": 0.0},  # Would need qrels
        "resource_usage": {

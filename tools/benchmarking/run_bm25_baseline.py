...
def run_pyserini_query(index_dir: Path, query_id: str, query_text: str, top_k: int = 100) -> List[Tuple[str, float]]:
...
def run_fallback_bm25_query(index_file: Path, query_text: str, top_k: int = 100) -> List[Tuple[str, float]]:
    ...
    for doc_id in index["doc_lengths"].keys():
        ...
        for token in tokens:
            if token in index["inverted_index"]:
                idf = np.log((len(index["doc_id_map"]) - len(index["inverted_index"][token]) + 0.5) / 
                            (len(index["inverted_index"][token]) + 0.5) + 1.0)
                
                tf = index["doc_term_counts"].get(str(doc_id), {}).get(token, 0)
                doc_scores[doc_id] = doc_scores.get(doc_id, 0.0) + \
                    idf * (tf * (k1 + 1)) / (tf + k1 * (1 - b + b * (doc_len / 1000)))
        
    # Sort and return top-k
    ranked = sorted(doc_scores.items(), key=lambda x: x[1], reverse=True)[:top_k]
    results = [(index["doc_id_map"].get(str(doc_id), str(doc_id)), score) for doc_id, score in ranked]
    return results


def calculate_recall_at_k(retrieved_ids: List[str], relevant_docs: Dict[str, int], k: int = 10) -> float:
    """Calculate recall@k: fraction of relevant docs in top-k retrieved."""
    
    relevant_at_k = sum(1 for doc_id in retrieved_ids[:k] if doc_id in relevant_docs and relevant_docs[doc_id] > 0)
...
def calculate_ndcg_at_k(retrieved_ids: List[str], relevant_docs: Dict[str, int], k: int = 10) -> float:
    """Calculate NDCG@k."""

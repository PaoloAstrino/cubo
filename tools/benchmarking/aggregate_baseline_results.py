...
def main():
    ...
    parser.add_argument(
        "--output-csv", type=Path, default=Path("results/baseline_comparison_table.csv")
    )
    parser.add_argument(
        "--output-latex", type=Path, default=Path("results/baseline_comparison_table.tex")
    )
    ...
    for dataset in args.datasets:
        ...
        for system, metrics in all_results[dataset].items():
            print(
                f"  {system:10} | p95={metrics['latency_p95_ms']:6.0f}ms | "

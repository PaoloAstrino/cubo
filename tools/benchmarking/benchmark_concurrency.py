...
async def simulate_query_workload(
    worker_id: int, num_queries: int, retriever, results_queue: asyncio.Queue
):
    """Simulate query workload for a single worker."""
    queries = [f"What is the meaning of document {i}?" for i in range(num_queries)]
    ...
    await results_queue.put(
        {"worker_id": worker_id, "completed_queries": len(latencies), "latencies_ms": latencies}
    )


async def simulate_ingestion_workload(num_documents: int, ingestor, results_queue: asyncio.Queue):
...
async def run_concurrent_benchmark(
    num_workers: int, queries_per_worker: int, concurrent_ingestion: bool = False
) -> Dict:
    """Run concurrent benchmark with multiple query workers and optional ingestion."""
    logger.info(
        f"Starting concurrency benchmark: {num_workers} workers, "
    ...
    if concurrent_ingestion:
        tasks.append(simulate_ingestion_workload(20, ingestor, results_queue))
...
def main():
    ...
    results = asyncio.run(
        run_concurrent_benchmark(
            args.num_workers, args.queries_per_worker, args.concurrent_ingestion
        )
    )

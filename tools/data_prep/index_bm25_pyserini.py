...
def convert_corpus_to_pyserini_format(corpus_file: Path, temp_dir: Path) -> Path:
    ...
    with open(corpus_file) as fin, open(output_file, "w") as fout:
        ...
        for line in fin:
            ...
            pyserini_doc = {"id": str(doc_id), "contents": content}
...
def build_index_with_pyserini(
    corpus_file: Path, output_dir: Path, memory_limit: str = "14G"
) -> bool:
...
def fallback_build_simple_bm25(corpus_file: Path, output_dir: Path) -> bool:
    ...
    index_data = {"doc_id_map": {}, "inverted_index": {}, "doc_term_counts": {}, "doc_lengths": {}}
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            corpus_converted, doc_count = convert_corpus_to_pyserini_format(
                args.corpus_file, Path(tmpdir)
            )
            success = build_index_with_pyserini(
                corpus_converted, args.output_dir, args.memory_limit
            )

            if success:
                print(f"\n[OK] Index ready at: {args.output_dir}")
                print(
                    f"  Index size: {sum(f.stat().st_size for f in args.output_dir.glob('**/*')) / (1024**2):.0f} MB"
                )
                return 0
            else:

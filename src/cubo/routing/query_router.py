...
class QueryRouter:
    ...
    _patterns = [
        (QueryType.TEMPORAL, re.compile(r"\b(when|date|year|in \d{4}|\d{4}-\d{4}|last|ago|recent|when did)\b", re.I)),
        (QueryType.COMPARATIVE, re.compile(r"\b(compare|vs\b|versus|which is better|is .* better)\b", re.I)),
        (QueryType.FACTUAL, re.compile(r"^(what is|who is|define|where is|how many|how much)\b", re.I)),
        (QueryType.CONCEPTUAL, re.compile(r"\b(what does .* mean|why|how (does|do))\b", re.I)),
        (QueryType.EXPLORATORY, re.compile(r"\b(explain|tell me about|overview|summary|describe)\b", re.I)),
    ]
    ...
    def __init__(self, presets: Optional[Dict[str, Any]] = None):
        ...
        self.confidence_threshold = float(config.get('query_router', {}).get('confidence_threshold', 0.6))
        self.llm_cache_ttl = int(config.get('query_router', {}).get('llm_cache_ttl_seconds', 300))
        self._llm_client = LLMQueryClassifier(model_override=config.get('query_router', {}).get('llm_model'), timeout_ms=int(config.get('query_router', {}).get('llm_timeout_ms', 1000))) if self.llm_enabled else None
            ...
            if pat.search(text):
                ...
                if confidence < self.confidence_threshold and self._llm_client:
                    ...
                    try:
                        ...
                        self._llm_cache[text] = {'label': label_str, 'confidence': conf_llm, 'reason': reason, 'ts': now}
    ...
    def extract_temporal_filter(self, query: str) -> Optional[Tuple[Optional[datetime], Optional[datetime]]]:
    ...
    def compute_strategy(self, query: str, context: Optional[dict] = None) -> RetrievalStrategy:
        ...
        return RetrievalStrategy(bm25_weight=bm25_weight, dense_weight=dense_weight,
                     use_reranker=use_reranker, k_candidates=k_candidates, temporal_range=temporal,
                     classification={'label': qtype.value, 'confidence': conf, 'fallback_used': fallback_used})

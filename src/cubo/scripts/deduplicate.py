...
from src.cubo.deduplication.semantic_deduplicator import DeduplicationResult, HybridDeduplicator
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--method', choices=['minhash', 'semantic', 'hybrid'], default=config.get('deduplication.method', 'hybrid'))
    parser.add_argument('--threshold', type=float, default=0.8, help='Jaccard similarity threshold for MinHash/prefilter.')
    parser.add_argument('--num-perm', type=int, default=128, help='Number of permutations for MinHash/prefilter.')
    parser.add_argument('--embeddings', help='Path to precomputed embeddings (required for semantic/hybrid).')
    parser.add_argument('--similarity-threshold', type=float, default=config.get('deduplication.similarity_threshold', 0.92))
    ...
    parser.add_argument('--representative-metric', default=config.get('deduplication.representative_metric', 'summary_score'))
    parser.add_argument('--disable-prefilter', action='store_true', help='Skip MinHash prefilter for semantic/hybrid runs.')
    parser.add_argument('--run-on', choices=['scaffold', 'chunks'], default=config.get('deduplication.run_on', 'scaffold'))

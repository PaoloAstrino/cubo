...
def main():
    ...
    parser.add_argument('--background', action='store_true', help='Run deep ingestion in the background (async)')
    args = parser.parse_args()

    manager = IngestionManager()
    result = manager.start_fast_pass(args.folder, args.output, skip_model=args.skip_model, auto_deep=(args.deep or args.background))
    if result:

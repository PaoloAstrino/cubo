...
class IngestionManager:
    ...
    def start_fast_pass(self, folder: str, output_dir: Optional[str] = None, skip_model: Optional[bool] = None, auto_deep: bool = False) -> Dict[str, Any]:
        ...
        try:
            result = build_bm25_index(folder, output_dir, skip_model=skip_model or config.get('ingestion.fast_pass.skip_heavy_models', True))
            # Update run details
            try:
                ...
                self.metadata.update_ingestion_status(run_id, 'fast_complete', started_at=datetime.utcnow().isoformat(), finished_at=datetime.utcnow().isoformat())
            except Exception:
    ...
    def start_deep_pass(self, folder: str, output_dir: Optional[str] = None, run_id: Optional[str] = None, resume: bool = False) -> Dict[str, Any]:
        ...
        self.metadata.update_ingestion_status(run_id, 'deep_started', started_at=datetime.utcnow().isoformat())
        try:
            dee = DeepIngestor(input_folder=folder, output_dir=output_dir)
            result = dee.ingest()
            if result:
                self.metadata.record_ingestion_run(run_id, folder, result.get('chunks_count', 0), result.get('chunks_parquet'))
                self.metadata.update_ingestion_status(run_id, 'deep_complete', finished_at=datetime.utcnow().isoformat())
            else:
                self.metadata.update_ingestion_status(run_id, 'deep_empty', finished_at=datetime.utcnow().isoformat())
            return {'run_id': run_id, 'result': result}
        except Exception as exc:
            logger.error(f"Deep ingress failed: {exc}")
            self.metadata.update_ingestion_status(run_id, 'failed', finished_at=datetime.utcnow().isoformat())
            return {'run_id': run_id, 'result': None}

...
class SemanticClusterer:
    ...
    def __init__(self, method: str = 'kmeans', min_cluster_size: int = 3, max_clusters: int = 50):
    ...
    def cluster_chunks(
        ...
        if len(embeddings) < self.min_cluster_size:
            logger.warning(f"Too few chunks ({len(embeddings)}) for clustering, using single cluster")
    ...
    def _auto_detect_clusters(self, embeddings: np.ndarray) -> int:
        ...
        if max_k < min_k:
            logger.warning(f"Not enough samples for clustering (need {self.min_cluster_size * min_k}, got {n_samples})")
            return 1
        ...
        logger.info(f"Auto-detected {best_k} clusters (silhouette score: {max(silhouette_scores):.3f})")

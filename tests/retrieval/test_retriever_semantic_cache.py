...
def test_retriever_uses_semantic_cache():
    ...
    retriever.semantic_cache = SemanticCache(ttl_seconds=600, similarity_threshold=0.9, max_entries=10, use_index=False)
    # Add a pre-cached result
    embedding = [1.0, 0.0, 0.0]
    cached_results = [{'document': 'Test doc', 'metadata': {'filename': 'test.txt'}, 'similarity': 0.95}]
    ...
    candidates = retriever._query_collection_for_candidates(query_embedding=embedding, initial_top_k=3, query='test-query')

...
class TestScaffoldGenerator(unittest.TestCase):
    ...
    def test_scaffold_grouping_logic(self):
        ...
        chunks_df = pd.DataFrame(
            {
                "chunk_id": ["c1", "c2", "c3"],
                "text": ["AI and machine learning", "AI applications", "Financial markets"],
            }
        )
    ...
    def test_save_and_load_scaffolds(self):
        ...
        chunks_df = pd.DataFrame({"chunk_id": ["c1", "c2"], "text": ["Chunk 1", "Chunk 2"]})
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(set(loaded_result["mapping"].keys()), set(result["mapping"].keys()))

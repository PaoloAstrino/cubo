...
def mock_hardware_avx2():
    return HardwareProfile(
        ...
        physical_cores=4, logical_cores=8, total_ram_gb=16,
        cpu_flags=["avx2"], blas_backend="mkl", allocator="libc"
    )
...
def test_model_loader_prefers_quantized_auto(mock_hardware_avx2):
    ...
    with patch("cubo.utils.hardware.detect_hardware", return_value=mock_hardware_avx2), \
         patch("cubo.embeddings.model_loader.config.get") as mock_config, \
         patch("cubo.monitoring.metrics.record") as mock_record, \
         patch("sentence_transformers.SentenceTransformer") as mock_st:
        
        # Setup config
        def config_side_effect(key, default=None):
            if key == "embeddings.prefer_quantized_cpu": return "auto"
            if key == "model_path": return "test-model"
            if key == "embeddings.device": return "cpu"
            return default
...
def test_model_loader_skips_quantized_no_avx(mock_hardware_no_avx):
    ...
    with patch("cubo.utils.hardware.detect_hardware", return_value=mock_hardware_no_avx), \
         patch("cubo.embeddings.model_loader.config.get") as mock_config, \
         patch("cubo.monitoring.metrics.record") as mock_record, \
         patch("sentence_transformers.SentenceTransformer") as mock_st:
...
def test_model_loader_force_quantized(mock_hardware_no_avx):
    ...
    with patch("cubo.utils.hardware.detect_hardware", return_value=mock_hardware_no_avx), \
        ...
        def config_side_effect(key, default=None):
            if key == "embeddings.prefer_quantized_cpu": return "always"
            if key == "model_path": return "test-model"
            if key == "embeddings.device": return "cpu"
            return default
        mock_config.side_effect = config_side_effect

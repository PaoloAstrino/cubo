...
pytestmark = pytest.mark.skipif(
    not PLOT_SCRIPT.exists(), reason=f"Plot script not found: {PLOT_SCRIPT}"
)
...
try:
    ...
    pytest.skip(
        "matplotlib not installed; skipping performance plot tests", allow_module_level=True
    )
...
def create_fake_results(tmp_path: Path):
        ...
        writer.writerow(
            {
                "run_id": "run1",
                "dataset": "smoke",
                "retrieval_config": "hybrid_test",
                "ablation": "none",
                "mode": "retrieval-only",
                "timestamp": 123,
                "success": 1.0,
                "avg_recall_at_k_10": 0.9,
                "avg_ndcg_at_k_10": 0.88,
                "avg_retrieval_latency_p50_ms": 60,
                "avg_answer_relevance": 0.9,
                "ingestion_minutes_per_gb": 1.5,
            }
        )
    ...
    run_json = {
        "metadata": {
            "run_id": "run1",
            "dataset": {"path": "data/smoke", "name": "smoke"},
            "retrieval_config": {"name": "hybrid_test"},
            "ablation": {"name": "none"},
            "mode": "retrieval-only",
            "timestamp": 123,
        ...
        "results": {
            "metadata": {"avg_recall_at_k_10": 0.9, "avg_retrieval_latency_p50_ms": 60},
            "easy": [],
        },
    }
...
def test_plot_results(tmp_path):
    ...
    assert (output_dir / "recall_vs_k_summary.png").exists() or (
        output_dir / "recall_vs_k.png"
    ).exists()

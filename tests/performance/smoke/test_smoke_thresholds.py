...
@pytest.mark.smoke
def test_retrieval_latency_and_recall_constraints(smoke_data_dir, sample_questions, sample_ground_truth):
    ...
    tester = RAGTester(str(sample_questions), str(smoke_data_dir), ground_truth_file=str(sample_ground_truth), mode='retrieval-only')

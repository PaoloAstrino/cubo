...
class TestGDPRCompliance:
    ...
    def test_query_scrubbing_enabled(self, gdpr_config):
        ...
        assert (
            scrubbed.startswith("sha256:") or len(scrubbed) == 64
    ...
    def test_no_plaintext_queries_in_logs(self, gdpr_config, tmp_path):
        ...
        for query in sensitive_queries:
            assert query.lower() not in log_content, f"Plaintext query found in logs: '{query}'"

        # Verify hashes ARE present
        assert "sha256:" in log_content or len(log_content) > 100, "No query hashes found in logs"

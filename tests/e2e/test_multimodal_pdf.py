...
class TestMultimodalPDFProcessing:
    ...
    def test_table_extraction_with_pdfplumber(self, pdf_with_table):
        ...
        with pdfplumber.open(pdf_with_table) as pdf:
            ...
            assert "2,100" in table_text or "2100" in table_text, "Total amount not found in table"
    ...
    def test_pdf_table_ingestion_and_query(self, pdf_with_table, tmp_path):
        ...
        for chunk in chunks:
            retriever.add_document(document=chunk["text"], metadata=chunk.get("metadata", {}))
        ...
        assert (
            "2,100" in results_text or "2100" in results_text
    ...
    def test_structured_data_preservation(self, pdf_with_table):
        ...
        with pdfplumber.open(pdf_with_table) as pdf:
            ...
            assert (
                "1,000" in row_text or "1000" in row_text
            ), "Price not associated with Software License"

            print(f"✓ Structured relationships preserved: {license_row}")

    @pytest.mark.skipif(
        importlib.util.find_spec("pytesseract") is None, reason="Tesseract OCR not available"
    )

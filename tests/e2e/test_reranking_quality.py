...
class TestRerankingQualityImpact:
    ...
    def test_reranking_improves_top_result(self, test_corpus, tmp_path):
        ...
        retriever_no_rerank = DocumentRetriever(model=model, use_reranker=False, top_k=5)
        ...
        retriever_with_rerank = DocumentRetriever(model=model, use_reranker=True, top_k=5)
        ...
        assert (
            relevant_count_with_rerank >= relevant_count_no_rerank
        ), f"Reranking didn't improve: {relevant_count_no_rerank} → {relevant_count_with_rerank}"

        print(
            f"✓ Reranking improved relevance: {relevant_count_no_rerank} → {relevant_count_with_rerank} relevant docs in top-3"
        )
    ...
    def test_reranking_filters_irrelevant_results(self, test_corpus, tmp_path):
        ...
        for doc in test_corpus:
            retriever.add_document(
                doc["text"], metadata={"id": doc["id"], "relevance": doc["relevance_to_ml"]}
            )
        ...
        assert (
            top_3_avg > bottom_2_avg
        ), f"Top-3 avg relevance ({top_3_avg}) not > bottom-2 ({bottom_2_avg})"

        print(
            f"✓ Reranking prioritizes relevant docs: top-3 avg={top_3_avg:.1f}, bottom-2 avg={bottom_2_avg:.1f}"
        )
    ...
    def test_recall_at_k_with_reranking(self, test_corpus):
        ...
        recall_with_rerank = len(set(retrieved_ids_with_rerank) & set(relevant_ids)) / len(
            relevant_ids
        )
        ...
        assert (
            recall_with_rerank >= recall_no_rerank - 0.1
        ), f"Reranking severely degraded recall: {recall_no_rerank:.2%} → {recall_with_rerank:.2%}"

        print(
            f"✓ Reranking impact: {(recall_with_rerank - recall_no_rerank)*100:+.1f}% recall change"
        )

...
def test_detect_blas_backend_numpy():
    ...
    config_mock.get_info.side_effect = lambda x: {"libraries": ["mkl_rt"]} if x == "mkl_info" else {}
...
def test_detect_hardware_integration():
    ...
    with patch("torch.cuda.is_available", return_value=False), \
         ...
         patch("cubo.utils.cpu_features.get_cpu_flags", return_value=["avx2"]), \
         patch("cubo.utils.cpu_features.get_topology", return_value={"physical_cores": 4, "logical_cores": 8}), \
         patch("cubo.utils.cpu_features.detect_blas_backend", return_value=("openblas", {})), \

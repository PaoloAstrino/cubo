...
def populated_store(tmp_path: Path) -> FaissStore:
    ...
    documents = [
        ...
        {
            "id": "research_1_chunk1",
            "content": "Machine learning models have shown remarkable progress in natural language understanding.",
            "filename": "ml_paper.pdf",
        },
        {
            "id": "research_1_chunk2",
            "content": "Transformer architectures enable parallel processing of sequences.",
            "filename": "ml_paper.pdf",
        },
        {
            "id": "research_2_chunk1",
            "content": "Reinforcement learning agents can learn complex behaviors through trial and error.",
            "filename": "rl_survey.pdf",
        },
        # Project documentation
        {
            "id": "project_1_chunk1",
            "content": "The API endpoints follow RESTful conventions with JSON responses.",
            "filename": "api_docs.md",
        },
        {
            "id": "project_1_chunk2",
            "content": "Authentication is handled via JWT tokens with 24-hour expiration.",
            "filename": "api_docs.md",
        },
        {
            "id": "project_2_chunk1",
            "content": "Database migrations should be run before deploying new versions.",
            "filename": "deployment.md",
        },
        # Meeting notes
        {
            "id": "meeting_1_chunk1",
            "content": "Q4 roadmap includes new dashboard features and performance improvements.",
            "filename": "meeting_notes.txt",
        },
        {
            "id": "meeting_2_chunk1",
            "content": "Budget allocation for cloud infrastructure approved by leadership.",
            "filename": "budget_meeting.txt",
        },
    ]
...
class TestCollectionWorkflow:
    ...
    def test_create_collection_and_add_documents(self, populated_store: FaissStore):
        ...
        research_coll = populated_store.create_collection(name="ML Research", color="#2563eb")
        ...
        result = populated_store.add_documents_to_collection(research_coll["id"], research_doc_ids)
    ...
    def test_multiple_collections_same_documents(self, populated_store: FaissStore):
        ...
        populated_store.add_documents_to_collection(
            all_papers["id"], ["research_1_chunk1", "research_1_chunk2", "research_2_chunk1"]
        )
        populated_store.add_documents_to_collection(
            ml_specific["id"], ["research_1_chunk1", "research_1_chunk2"]  # Only ML paper, not RL
        )
    ...
    def test_query_filtering_by_collection(self, populated_store: FaissStore):
        ...
        populated_store.add_documents_to_collection(
            project_coll["id"], ["project_1_chunk1", "project_1_chunk2", "project_2_chunk1"]
        )
    ...
    def test_collection_deletion_preserves_documents(self, populated_store: FaissStore):
        ...
        populated_store.add_documents_to_collection(
            temp_coll["id"], ["meeting_1_chunk1", "meeting_2_chunk1"]
        )
    ...
    def test_collection_list_ordering(self, populated_store: FaissStore):
        ...
        _coll_a = populated_store.create_collection(name="First")
        time.sleep(0.01)
        _coll_b = populated_store.create_collection(name="Second")
        time.sleep(0.01)
        _coll_c = populated_store.create_collection(name="Third")
    ...
    def test_remove_documents_from_collection(self, populated_store: FaissStore):
        ...
        populated_store.add_documents_to_collection(
            coll["id"], ["research_1_chunk1", "research_1_chunk2", "research_2_chunk1"]
        )

        # Remove one document
        removed = populated_store.remove_documents_from_collection(
            coll["id"], ["research_1_chunk2"]
        )

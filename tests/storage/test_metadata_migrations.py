...
def test_migrate_scaffold_mappings_add_run_id(tmp_path: Path):
    ...
    cur = conn.cursor()
    cur.execute(
        """CREATE TABLE scaffold_mappings (scaffold_id TEXT, chunk_id TEXT, metadata TEXT, PRIMARY KEY (scaffold_id, chunk_id))"""
    )
    cur.execute(
        """INSERT INTO scaffold_mappings (scaffold_id, chunk_id, metadata) VALUES ('s1', 'c1', '{}')"""
    )
    conn.commit()
    conn.close()
    ...
    cur.execute(
        "SELECT run_id, scaffold_id, chunk_id FROM scaffold_mappings WHERE scaffold_id='s1' and chunk_id='c1'"
    )
    row = cur.fetchone()

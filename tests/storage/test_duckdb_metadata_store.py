...
class TestDocumentOperations:
    ...
    def test_add_multiple_documents(self, store):
        ...
        docs = [
            DocumentMetadata(doc_id=f"doc_{i}", text=f"Document {i}")
            for i in range(5)
        ]
    ...
    def test_delete_multiple_documents(self, store):
        ...
        docs = [
            DocumentMetadata(doc_id=f"del_{i}", text=f"Delete {i}")
            for i in range(3)
        ]
    ...
    def test_mark_indexed(self, store):
        ...
        docs = [
            DocumentMetadata(doc_id=f"idx_{i}", text=f"Index {i}")
            for i in range(3)
        ]
        store.add_documents(docs)
...
class TestQueryOperations:
    ...
    def test_query_all_documents(self, store):
        ...
        docs = [
            DocumentMetadata(doc_id=f"q_{i}", text=f"Query doc {i}")
            for i in range(10)
        ]
        store.add_documents(docs)
    ...
    def test_query_with_list_filter(self, store):
        ...
        docs = [
            DocumentMetadata(doc_id=f"list_{i}", text=f"List doc {i}")
            for i in range(5)
        ]
        store.add_documents(docs)
    ...
    def test_query_with_limit_offset(self, store):
        ...
        docs = [
            DocumentMetadata(doc_id=f"page_{i}", text=f"Page doc {i}")
            for i in range(25)
        ]
        store.add_documents(docs)
...
class TestRawSQLQuery:
    ...
    def test_execute_query(self, store):
        ...
        docs = [
            DocumentMetadata(doc_id=f"sql_{i}", text=f"SQL doc {i}")
            for i in range(5)
        ]
        store.add_documents(docs)
    ...
    def test_execute_query_with_params(self, store):
        ...
        docs = [
            DocumentMetadata(doc_id=f"param_{i}", text=f"Param doc {i}")
            for i in range(5)
        ]
        store.add_documents(docs)
...
class TestEdgeCases:
    ...
    def test_concurrent_operations(self, reset_singleton):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            threads = [
                threading.Thread(target=add_docs, args=(i,))
                for i in range(3)
            ]

...
def test_semantic_cache_linear_lookup():
    cache = SemanticCache(ttl_seconds=10, similarity_threshold=0.99, max_entries=10, use_index=False)
...
def test_semantic_cache_faiss_lookup():
    cache = SemanticCache(ttl_seconds=10, similarity_threshold=0.9, max_entries=10, use_index=True, index_type='hnsw', hnsw_m=8)

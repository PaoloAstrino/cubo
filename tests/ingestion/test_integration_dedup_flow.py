...
def test_end_to_end_dedup_and_index_flow(tmp_path: Path, monkeypatch):
    ...
    assert set(retriever.dedup_canonical_lookup.values()).issubset(
        set(payload["canonical_map"].values())
    )
...
def test_end_to_end_faiss_build_and_retrieval(tmp_path: Path):
    ...
    manager = FAISSIndexManager(
        dimension=emb.shape[1], index_dir=out_dir, nlist=4, hnsw_m=8, hot_fraction=0.5
    )
    ...
    payload = json.loads(open(str(map_path), encoding="utf-8").read())
    expected_reps = set(
        str(rep.get("chunk_id")) for rep in payload.get("representatives", {}).values()
    )
    ...
    assert saved_ids.intersection(
        returned_ids
    ), f"Expected at least one representative to be returned; got {returned_ids}"
...
def test_cli_build_faiss_and_retrieve(tmp_path: Path):
    ...
    payload = json.loads(open(str(meta), encoding="utf-8").read())

...
def test_deep_ingestor_uses_parallel_execution():
    ...
    with patch("cubo.ingestion.deep_ingestor.ProcessPoolExecutor") as mock_executor_cls, patch(
        "cubo.ingestion.deep_ingestor.as_completed"
    ) as mock_as_completed, patch("cubo.ingestion.deep_ingestor.get_metadata_manager"), patch(
        "pathlib.Path.exists", return_value=True
    ), patch(
        "os.walk", return_value=[("/tmp", [], ["file1.txt", "file2.txt"])]
    ), patch(
        "cubo.ingestion.deep_ingestor.DeepIngestor._process_file", return_value=[{"text": "foo"}]
    ):
...
def test_deep_ingestor_serial_fallback():
    ...
    with patch("cubo.ingestion.deep_ingestor.ProcessPoolExecutor") as mock_executor_cls, patch(
        "cubo.ingestion.deep_ingestor.get_metadata_manager"
    ), patch("pathlib.Path.exists", return_value=True), patch(
        "os.walk", return_value=[("/tmp", [], ["file1.txt"])]
    ), patch(
        "cubo.ingestion.deep_ingestor.DeepIngestor._process_file", return_value=[{"text": "foo"}]
    ):
